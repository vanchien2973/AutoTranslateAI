...
public class RenderStepTests
{
    ...
    {
        var step = new RenderStep(Substitute.For<IVideoRenderer>(), StepFixture.Workspace());
    ...
    public async Task Given_DubbingEnabledButNoAudio_When_Execute_Then_Fails()
    {
        ...
        var result = await new RenderStep(Substitute.For<IVideoRenderer>(), StepFixture.Workspace())
            .ExecuteAsync(context, CancellationToken.None);
    ...
    public async Task Given_SourceAndAudio_When_Execute_Then_RendersWithMixedAudio()
    {
        ...
        var result = await new RenderStep(renderer, StepFixture.Workspace()).ExecuteAsync(context, CancellationToken.None);
